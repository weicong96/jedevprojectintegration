...
<html>
	<head>
		<title>Retrieve Consultation Slots</title>
...
Welcome,<%=username %>
<table align="center">
<tr><td align="center"><h1>Retrieve Consultation Slots</h1></td></tr>
</table>
...
<form action="BUpdateBookingServlet" method="POST">
<br/>
...
<tr><td></td><td><%=bookingList.get(i).getBookVenue()%></td><td><%=bookingList.get(i).getBookDate()%></td><td><%=bookingList.get(i).getBookTime()%></td>
		<td><%if(bookingList.get(i).getFk_groupCode()==null){%>No Group
			<%}else{%>
			<%=bookingList.get(i).getFk_groupCode()%>
			<%} %></td><td><%=bookingList.get(i).getFk_staffID()%></td><td>
